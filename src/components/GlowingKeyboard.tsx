...
const iconSize = 8; // reduced from 12
    ...
    ({
        ...
        return (
            <div
                ...
                className="card-color flex w-[490px] flex-col gap-1.5 rounded-md p-1.5" // reduced width and padding
            >
        ...
        return (
            <motion.div
                ...
                style={{
                    ...
                    color: data.shouldGlow ? `${glowColor}50` : "#ffffff60",
                }}
                className={cn(
                    className,
                    data.shouldGlow
                    ...
                    "shadow-[inset_0px_0px_10px_rgba(0,0,0,0.2)] group relative flex select-none flex-col items-center justify-center gap-1 rounded-md border border-x-[1px] border-b-[0.1px] border-t-[1.5px] border-border/80 px-1.5 duration-200 first:items-start last:items-end active:!scale-[0.98]",
                    transparentKey ? "bg-transparent" : "bg-[#121214]"
                )}
            >
                {!isSingleKey && (
                    <div className="mt-0.5 h-[7px] w-[7px] text-center text-[10px] font-light text-muted-foreground/70 duration-200 group-hover:text-muted-foreground">
                        {icon}
                    </div>
                )}
                <div
                    ...
                    className={cn(
                        isLastRow && "!text-[6px]",
                        isSingleKey ? "text-xs" : "text-[10px]",
                        data.shouldGlow &&
                            "animate-[6000ms] drop-shadow-[2px_2px_6px_#f43f5d] blur-in",
                        "text-center text-muted-foreground/70 duration-200 group-hover:text-white"
                    )}
                >
                    {data.text}
                </div>
    ...
    ({
        ...
        return (
            <motion.div
                ...
                className={cn(
                    "shadow-[inset_0px_0px_10px_rgba(0,0,0,0.2)] group relative flex flex-col items-center justify-center gap-1 rounded-md border border-x-[1px] border-b-[0.1px] border-t-[1.5px] border-border/80 duration-200 hover:border-white/20",
                    transparentKey ? "bg-transparent" : "bg-[#121214]"
                )}
            >
                <div className="shadow-[inset_0px_0px_1.5px_rgba(0,0,0,0.6)] h-5 w-5 rounded-full bg-muted/20 duration-200 group-hover:bg-muted/35" />
            </motion.div>
        );
    }
);
        ...
        return (
            <div className="flex flex-row gap-1.5">
                <IconKeyboardKey
                    ...
                    rowIndex={rowIndex}
                    isSingleKey
                    className="!h-1/2 translate-y-3 !items-center"
                    icon={null}
...
// The rest of the key data arrays remain the same, but update iconSize where used:
...
const fourthRowData: KeyRowDataProps[] = [
    {
        icon: <DotIcon size={iconSize + 2} className="-ml-1 -mt-0.5" />, // reduced from +3
        text: "caps lock",
        className: "!basis-full",
    },
    {
...
const sixthRowData: KeyRowDataProps[] = [
    ...
    },
    {
        icon: <CommandIcon size={iconSize} />,
        text: "command",
        className: "!items-start !basis-14",
    },
    {
        icon: <OptionIcon size={iconSize} />,
        text: "option",
